...
router.post('/', async function(req, res) {
    ...
    try {
        const existingVehicle =
            await vehiclesModel.getByIssuedStateAndLicensePlate(
                req.body.license_issued_state,
        ...
        if (existingVehicle.length > 0) {
            return res
                ...
                .json({
                    status: 'failed',
                    message:
                        'Uh oh, this vehicle is already registered in our system, please ask the primary owner to add you as a co-owner.',
                });
        }
        ...
        return res
            ...
            .json({
                status: 'success',
                message: 'Successfully inserted vehicle',
            });
    } catch (err) {
        return res
            .status(500)
            .json({
                err,
                message: 'Unable to insert vehicle due to server errors',
            });
    }
});
...
router.put('/accept', async function(req, res) {
    ...
    try {
        const vehicle_ownership =
            await vehicleOwnershipModel.getByUserIdAndVehicleId(id, vehicle_id);
        ...
        await vehicleOwnershipModel.updateById(vehicle_ownership[0].id, {
            status: 'ACCEPTED',
        });
        return res
            .status(200)
        ...
        return res
            ...
            .json({
                err,
                message:
                    'Unable to accept vehicle ownership due to server errors',
            });
    }
});
...
router.get('/ownership/:vehicle_id', async function(req, res) {
    ...
    try {
        ...
        if (isUserOwnVehicle) {
            const vehicle_ownership_records =
                await vehicleOwnershipModel.getByVehicleIdJoinUser(vehicle_id);
            return res
                .status(200)
                .json({
                    status: 'success',
                    vehicle_ownership_records,
                    message: 'Successfully retrieved ownership info',
                });
        } else {
            return res
                .status(401)
    ...
    try {
        if (email === invitee_email) {
            return res
                ...
                .json({
                    status: 'failed',
                    message: 'Unauthorized action, cannot invite yourself',
                });
        }
        const ownershipInfo =
            await vehicleOwnershipModel.getByUserIdAndVehicleId(id, vehicle_id);
        const isOwnershipExist = ownershipInfo.length > 0;
        const isUserPrimaryOwner = isOwnershipExist ?
            ownershipInfo[0].is_primary_owner :
            false;
        ...
        await mailer.sendCoownVehicleInvitation(
            userInfo[0],
            inviteeInfo[0],
            vehicleInfo[0],
        ...
        return res
            ...
            .json({
                status: 'success',
                message: 'Successfully inserted ownership record',
            });
    } catch (err) {
        return res
            .status(500)
            .json({
                err,
                message:
                    'Unable to insert vehicle ownership due to server errors',
            });
    }
});
...
router.put('/transfer_ownership', async function(req, res) {
    ...
    try {
        const user_ownership =
            await vehicleOwnershipModel.getByUserIdAndVehicleId(id, vehicle_id);
        const isAuthorized =
            user_ownership.length !== 0 && user_ownership[0].is_primary_owner;
        if (!isAuthorized) {
            return res
                .status(401)
                .json({status: 'failed', message: 'Unauthorized action'});
        ...
        if (!isInviteeOwnershipExist) {
            return res
                ...
                .json({
                    status: 'failed',
                    message: 'Ownership information not found',
                });
        }
        ...
        if (invitee_ownership[0].status !== 'ACCEPTED') {
            return res
                ...
                .json({
                    status: 'failed',
                    message: 'Invitee not yet accepted invite as a co-owner',
                });
        }

        await vehicleOwnershipModel.updateById(user_ownership[0].id, {
            is_primary_owner: 0,
        });
        await vehicleOwnershipModel.updateById(invitee_ownership[0].id, {
            is_primary_owner: 1,
        });
        ...
        return res
            ...
            .json({
                status: 'success',
                message: 'Successfully transfer ownership record',
            });
    } catch (err) {
        return res
            .status(500)
            .json({
                err,
                message:
                    'Unable to change vehicle ownership due to server errors',
            });
    }
});
...
router.put('/:vehicle_id', async function(req, res) {
    ...
    try {
        ...
        const isUserAuthorized =
            vehicleOwnerRecords.length > 0 &&
            vehicleOwnerRecords[0].is_primary_owner;
        if (!isUserAuthorized)
            return res
                .status(401)
                .json({status: 'failed', message: 'Unauthorized action'});
        ...
        return res
            .status(200)
            .json({status: 'success', message: 'Successfully updated vehicle'});
    } catch (err) {
        return res
            .status(500)
            .json({
...
router.delete('/delete_ownership/:ownership_id', async function(req, res) {
    ...
    try {
        const ownership_info = await vehicleOwnershipModel.getById(
            ownership_id,
        );
        ...
        const isPrimaryOwner =
            primary_owner.length !== 0 && primary_owner[0].is_primary_owner;
        if (!isPrimaryOwner) {
            return res
                .status(401)
                .json({status: 'failed', message: 'Unauthorized action'});
        ...
        return res
            ...
            .json({
                status: 'success',
                message: 'Successfully deleted ownership record',
            });
    } catch (err) {
        return res
            .status(500)
...
router.delete('/co_owner_remove/:vehicle_id', async function(req, res) {
    ...
    try {
        ...
        const isOwnershipAndVehicleExist =
            ownership_info.length !== 0 && vehicle_info.length !== 0;
        if (!isOwnershipAndVehicleExist) {
            return res
                .status(404)
                .json({
                    status: 'failed',
                    message: 'Ownership or vehicle information not found',
                });
        }
        ...
        return res
            ...
            .json({
                status: 'success',
                message: 'Successfully deleted vehicle and ownership record',
            });
    } catch (err) {
        return res
            .status(500)
            .json({
                err,
                message: 'Unable to delete vehicle due to server errors',
            });
    }
});
